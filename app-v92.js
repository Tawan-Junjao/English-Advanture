...
const KEY="englishAdventureV92Complete";
...
let voiceName=localStorage.getItem("voiceNameV92")||"";
let voiceRate=Number(localStorage.getItem("voiceRateV92")||.72);
let speechRun=0;
...
function speakWord(text,rate=voiceRate){
  stopSpeech();
  const run=speechRun;
  return new Promise(resolve=>{
    const u=makeSpeech(String(text).toLowerCase(),rate);
...
async function spellWord(word){
  ...
  for(const ch of String(word).toUpperCase()){
    ...
    const name=LETTER_NAME[ch];
    if(!name)continue;
    await new Promise(resolve=>{
      const u=makeSpeech(name,Math.max(.6,voiceRate-.08));
      u.onend=resolve;u.onerror=resolve;
...
function next(){clearInterval(monster.timer);$("#monsterHud").hidden=true;lock=false;q++;current=pick();$("#progress").textContent=`ข้อ ${q}/${total}`;$("#feedback").textContent="";if(mode==="picture"){$("#prompt").textContent="รูปนี้คือคำว่าอะไร?";$("#visual").textContent=current.emoji;renderButtons(opts(current),current.word);speakWord(current.word)}else if(mode==="listen"){$("#prompt").textContent="ฟังแล้วแตะรูป";$("#visual").textContent="🔊";renderButtons(opts(current),current.word,x=>`${x.emoji}<br><small>${x.word}</small>`);speakWord(current.word)}else if(mode==="spell")renderSpell()}
...
function correct(){if(lock)return;lock=true;rec(true);score++;$("#feedback").textContent="เก่งมาก! ⭐";speakWord("great job",.82);setTimeout(()=>q>=total?finish():next(),850)}
...
function resetMonsterScene(){
  const hero=$("#heroChar"),enemy=$("#monsterChar"),fill=$("#dangerFill");
  if(!hero||!enemy)return;
  hero.classList.remove("caught");
  ...
  requestAnimationFrame(()=>{
    hero.style.transition="left .42s ease";
    enemy.style.transition="left .42s ease";
  });
}
...
$("#voiceSelect").onchange=e=>voiceName=e.target.value;$("#voiceRate").oninput=e=>{voiceRate=Number(e.target.value);$("#rateValue").textContent=voiceRate.toFixed(2)};$$("[data-test]").forEach(b=>b.onclick=()=>speakWord(LETTER_NAME[b.dataset.test],voiceRate));$("#saveVoice").onclick=()=>{localStorage.setItem("voiceNameV92",voiceName);localStorage.setItem("voiceRateV92",voiceRate);$("#voiceStatus").textContent="บันทึกแล้ว ✅"};
...
$$(".back").forEach(b=>b.onclick=()=>{clearInterval(monster.timer);stopSpeech();resetMonsterScene();show("home")});
$("#wordSound").onclick=()=>current&&speakWord(current.word);$("#spellSound").onclick=()=>current&&spellWord(current.word);$("#reset").onclick=()=>{if(confirm("ล้างข้อมูลทั้งหมดหรือไม่?")){progress={};save();parentPage()}};
save();
