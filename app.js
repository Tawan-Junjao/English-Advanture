...
async function sayWord(word){
  stopAudio();
  const run=audioRun;
  await playFileWithoutStopping(wordAudioPath(word),run);
}
...
async function spellWord(word){
  ...
  for(let i=0;i<letters.length;i++){
    ...
    await playFileWithoutStopping(letterAudioPath(letters[i]),run);
    if(run!==audioRun)return;
