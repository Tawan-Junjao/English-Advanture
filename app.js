...
const KEY="englishAdventureV9_1Complete";
...
let voiceName=localStorage.getItem("voiceNameV9")||"";
let voiceRate=Number(localStorage.getItem("voiceRateV9")||.72);
let speechSequence=0;

function voices(){
  return speechSynthesis.getVoices().filter(v=>/^en/i.test(v.lang)||/English/i.test(v.name));
}
function selectedVoice(){
  const vs=voices();
  return vs.find(v=>v.name===voiceName)||
         vs.find(v=>/Samantha|Ava|Allison|Susan/i.test(v.name))||
         vs.find(v=>v.lang==="en-US")||vs[0];
}
function makeUtterance(text,rate=voiceRate){
...
function speak(text,rate=voiceRate){
  speechSequence++;
  const run=speechSequence;
  ...
  return new Promise(resolve=>{
    ...
    u.onerror=()=>resolve();
    setTimeout(()=>{
      if(run===speechSequence)speechSynthesis.speak(u);
      else resolve();
    },90);
  });
}
function speakLetter(letter){
  const name=LETTER_NAME[String(letter).toUpperCase()];
  if(!name)return Promise.resolve();
  return speak(name,Math.max(.62,voiceRate-.05));
}
async function spell(word){
  ...
  for(const letter of letters){
    if(run!==speechSequence)return;
    const name=LETTER_NAME[letter];
    if(!name)continue;
    await new Promise(resolve=>{
      const u=makeUtterance(name,Math.max(.60,voiceRate-.08));
      u.onend=resolve;
      u.onerror=resolve;
...
function next(){clearInterval(monster.timer);$("#monsterHud").hidden=true;lock=false;q++;current=pick();$("#progress").textContent=`ข้อ ${q}/${total}`;$("#feedback").textContent="";if(mode==="picture"){$("#prompt").textContent="รูปนี้คือคำว่าอะไร?";$("#visual").textContent=current.emoji;renderButtons(opts(current),current.word);speak(current.word)}else if(mode==="listen"){$("#prompt").textContent="ฟังแล้วแตะรูป";$("#visual").textContent="🔊";renderButtons(opts(current),current.word,x=>`${x.emoji}<br><small>${x.word}</small>`);speak(current.word)}else if(mode==="spell")renderSpell()}
function renderSpell(){$("#prompt").textContent="ฟังเสียงสะกด แล้วเรียงตัวอักษร";$("#visual").textContent=current.emoji;let built=[];$("#answerArea").innerHTML=`<div style="grid-column:1/-1"><div class="slots">${current.word.split("").map(()=>'<div class="slot"></div>').join("")}</div><div class="letters">${shuffle(current.word.toUpperCase().split("")).map(l=>`<button class="letter-btn" data-l="${l}">${l}</button>`).join("")}</div></div>`;$$(".letter-btn").forEach(b=>b.onclick=()=>{if(b.disabled)return;const exp=current.word.toUpperCase()[built.length];if(b.dataset.l===exp){b.disabled=true;built.push(exp);$$(".slot")[built.length-1].textContent=exp;speakLetter(exp);if(built.length===current.word.length)correct()}else wrong(b)});setTimeout(()=>spell(current.word),250)}
function correct(){if(lock)return;lock=true;rec(true);score++;$("#feedback").textContent="เก่งมาก! ⭐";speak("Great job");setTimeout(()=>q>=total?finish():next(),850)}
...
$("#voiceSelect").onchange=e=>voiceName=e.target.value;$("#voiceRate").oninput=e=>{voiceRate=Number(e.target.value);$("#rateValue").textContent=voiceRate.toFixed(2)};$$("[data-test]").forEach(b=>b.onclick=()=>speakLetter(b.dataset.test));$("#saveVoice").onclick=()=>{localStorage.setItem("voiceNameV9",voiceName);localStorage.setItem("voiceRateV9",voiceRate);$("#voiceStatus").textContent="บันทึกแล้ว ✅"};
...
$$(".back").forEach(b=>b.onclick=()=>{clearInterval(monster.timer);speechSequence++;speechSynthesis.cancel();$("#heroChar").style.left="7%";$("#monsterChar").style.left="84%";show("home")});
$("#wordSound").onclick=()=>current&&speak(current.word);$("#spellSound").onclick=()=>current&&spell(current.word);$("#reset").onclick=()=>{if(confirm("ล้างข้อมูลทั้งหมดหรือไม่?")){progress={};save();parentPage()}};
save();if("serviceWorker"in navigator)navigator.serviceWorker.register("sw.js?v=90");
